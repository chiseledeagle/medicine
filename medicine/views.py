...
def searchco(request):
    if request.method == 'POST':
        searchco = request.POST['searchco']
        url = 'https://www.ask.com/web?q=what+is'+searchco+'+and+its+symptoms'
        res = requests.get(url)
        soup = bs(res.text, 'lxml')
...
def searchs(request):
    if request.method == 'POST':
        searchco1 = request.POST['searchco1']
        searchco2 = request.POST['searchco2']
        searchco3 = request.POST['searchco3']
        searchco4 = request.POST['searchco4']
        url = 'https://www.ask.com/web?q=I+have+'+searchco1+'+,+'+searchco2+'+,+'+searchco3+'+and+'+searchco4+'+What+does+it+mean+?'
        res = requests.get(url)
        soup = bs(res.text, 'lxml')
